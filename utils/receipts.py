...
def build_disbursement_receipt_pdf(*, receipt_id: str, borrower_name: str, lender_name: str,
                                    loan_reference: str, amount: float, platform_fee: float,
                                    disbursed_at: str) -> bytes:
    buf = BytesIO()
    doc = SimpleDocTemplate(buf, pagesize=A4, topMargin=30 * mm, bottomMargin=20 * mm)
    styles = getSampleStyleSheet()
    title_style = ParagraphStyle("MpolaTitle", parent=styles["Heading1"], textColor=colors.HexColor("#1B2B3A"))
    sub_style = ParagraphStyle("MpolaSub", parent=styles["Normal"], textColor=colors.HexColor("#C4A55A"), fontSize=11)

    elements = [
        Paragraph("Mpola", title_style),
        Paragraph("Peer-to-Peer Lending Platform — Disbursement Receipt", sub_style),
        Spacer(1, 12 * mm),
    ]
    ...
    rows = [
        ...
        ["Borrower", borrower_name],
        ["Lender", lender_name],
        ["Platform Fee (Lender)", f"UGX {platform_fee:,.0f}"],
        ["Amount Disbursed", f"UGX {amount:,.0f}"],
    ]
    table = Table(rows, colWidths=[50 * mm, 100 * mm])
    table.setStyle(TableStyle([
        ("FONTSIZE", (0, 0), (-1, -1), 10),
    ...
    elements.append(Paragraph(
        "This is a system-generated receipt from Mpola Uganda Ltd. "
        "For questions about this disbursement, contact support from the app.",
        ParagraphStyle("Footer", parent=styles["Normal"], textColor=colors.HexColor("#8A9BA8"), fontSize=8),
    ))
